        ...
        <div className="max-w-md w-full bg-white p-8 rounded-lg shadow-xl border border-emerald-100/50 space-y-8 relative overflow-hidden">
          ...
          {otpStep === 'PHONE' ? (
            <form onSubmit={handleRequestOTP} className="space-y-6">
              ...
              {error === "NON_INSCRIT" && (
                 <div className="p-5 bg-emerald-50 border border-emerald-100 rounded-md flex flex-col items-center gap-3 text-center">
                    <div className="w-10 h-10 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center">
                      <UserPlus className="w-5 h-5" />
                    </div>
                    <div>
                      <h4 className="text-emerald-800 font-bebas text-lg tracking-widest">NOUVEAU CLIENT ?</h4>
                      <p className="text-emerald-600 text-[11px] font-bold mt-1">
                        Pour créer votre compte sécurisé, vous devez d'abord prendre votre premier rendez-vous.
                      </p>
                    </div>
                    <a href="/#rendez-vous" className="mt-2 inline-flex items-center gap-2 px-6 py-2.5 bg-emerald-600 text-white rounded-md font-bebas tracking-widest text-sm hover:bg-emerald-700 transition-colors">
              ...
              {error !== "NON_INSCRIT" && (
                <div className="text-center pt-2">
                   ...
                   <a 
                    href="/#rendez-vous"
                    className="inline-block px-6 py-2 border-2 border-slate-900 text-slate-900 rounded-md font-bebas tracking-widest uppercase text-xs hover:bg-slate-900 hover:text-white transition-all"
                   >
                    ...
                    clientData.reparations.filter((r: Repair) => r.statut !== 'TERMINE' && r.statut !== 'ANNULE').map((r: Repair) => (
                      <div key={r.id} className="p-6 bg-white rounded-lg border border-slate-200 shadow-sm hover:border-emerald-500 transition-all group relative overflow-hidden">
                        ...
                        <div className="space-y-4 relative z-10">
                          <h4 className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Historique d'intervention</h4>
                          <div className="space-y-3 pl-2 border-l-2 border-emerald-100/50">
                            {r.etapes && r.etapes.length > 0 ? (
                              r.etapes.map(etape => (
                                <div key={etape.id} className="relative pl-4">
                                  <div className="absolute -left-[23px] top-1.5 w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]"></div>
                                  <p className="text-sm font-bold text-slate-700">{etape.description}</p>
                                  <p className="text-[9px] text-slate-400 font-bold mt-0.5 uppercase tracking-widest">{format(new Date(etape.date_ajout), "d MMM yyyy 'à' HH:mm", { locale: fr })}</p>
                                </div>
                              ))
                            ) : (
                              <p className="text-[10px] text-slate-400 italic">En attente de mise à jour de l'atelier...</p>
                            )}
                          </div>
                        </div>
                      </div>
              ...
              {clientData.reparations.map((r: Repair) => (
                <div key={r.id} className="p-6 bg-white rounded-lg border border-slate-200 shadow-sm flex flex-col md:flex-row gap-6 items-start md:items-center group hover:border-emerald-500 transition-all">
                  ...
                  <div className="flex-1 space-y-1">
                    ...
                    <p className="text-xs text-slate-500 font-medium italic opacity-70 mb-4">"{r.description}"</p>
                    
                    {r.etapes && r.etapes.length > 0 && (
                      <div className="mt-4 p-4 bg-slate-50/50 rounded-xl border border-slate-100/50">
                        <p className="text-[9px] font-black uppercase tracking-widest text-emerald-600 mb-3">Chronologie</p>
                        <div className="space-y-3 pl-2 border-l-2 border-emerald-200/50">
                          {r.etapes.map(etape => (
                            <div key={etape.id} className="relative pl-4">
                              <div className="absolute -left-[23px] top-1.5 w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]"></div>
                              <p className="text-xs font-bold text-slate-700">{etape.description}</p>
                              <p className="text-[8px] text-slate-400 font-bold mt-0.5 uppercase tracking-widest">{format(new Date(etape.date_ajout), "d MMM yyyy 'à' HH:mm", { locale: fr })}</p>
                            </div>
                          ))}
                        </div>
                      </div>
            ...
            <div className="space-y-4 animate-in slide-in-from-bottom-4 duration-700">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-2">
                ...
                <div className={`p-5 rounded-xl text-center col-span-2 md:col-span-1 ${clientData.solde_impaye > 0 ? 'bg-rose-50 text-rose-700' : 'bg-slate-50 text-slate-500'}`}>
                  ...
                  <p className="text-2xl font-bebas tracking-wider">
                    {Math.max(0, Number(clientData.solde_impaye || 0)).toLocaleString('fr-FR')}
                      ...
                      <div className="flex items-start gap-4 flex-1">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {inv.demande_normalisation && !inv.is_normalised && (
                              <span className="flex items-center gap-1 px-2 py-0.5 bg-blue-50 text-blue-600 rounded text-[8px] font-bold animate-pulse uppercase">
                                <RefreshCw className="w-2 h-2 animate-spin" /> Normalisation en cours...
                              </span>
                            )}
                            {inv.is_normalised && (
                              <span className="px-2 py-0.5 bg-indigo-50 text-indigo-600 rounded text-[8px] font-bold uppercase tracking-widest border border-indigo-100">
                                ✓ e-MECeF
                              </span>
                            )}
                          </div>
